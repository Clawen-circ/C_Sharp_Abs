...
```
## С плавающей точкой
Тип значений `float` ограничен промежутком __5e-45 ; 3,4e+38 (32bit)__.
Тип значений `double` ограничен промежутком __5e-324 ; 1,7e+308 (64bit)__ применяется чаще, потому что используется в математических операциях. 
...
Это тип `decimal`. Он применяется для финансовых расчётов, так как обладает наивысшей точностью среди типов данных __1e-28 ; 7,9e+28 (128bit__, вплоть до 28 знаков после запятой). 
...
```
Важным моментом является наличие суффикса `m` в значениях `decimal`. Суффикс необходим, лишь если задаваемое значение – число с плавающей точкой (тогда это значение будет интерпритировано, как обычная числовая константа с плавающей точкой). Можно присвоить переменной типа `decimal` целочисленное значение без `m`.
...
В отличие от многих других я зыков в __C#__ используется не __8 а 16 битный символьный тип данных__. Это, так называемый, _Unicode_ , который включает символы из почти всех существующих алфавитов (8bit – это ASCII).
В сравненнии __8bit – 0 ; 127 16bit – 0 ; 65535__. _ASCII_ является подмножеством юникода, так что он остаёся действительным в __C#__. Тип `char` целочисленный но он не принимает значения которые нельзя конвертировать в символ, например:
```csharp
char ch;
...
Тип `bool` принимает два значения `true` и `false`. В Шарпе __нет возможности__ преобразовать значения 1 и 0 в значения `true` и `false` соответственно. 
...
```
Невозможно преобразование типов `float`, `double`, `decimal`, а также числовых `bool` и `char`. Также `bool` и `char` несовместимы.

Преобразование типов происходит не только в присвоениях, но и в выражениях.
Алгоритм, определяющий приведения:

Если один операнд имеет значение `decimal`, то второй становится `decimal`. Исключение: Второй не должен быть `float` или `double`;

ЕСЛИ один операнд имеет значение `double`, то второй становится `double`;

ЕСЛИ один операнд имеет значение `float`, то второй становится `float`;

ЕСЛИ один операнд имеет значение `long`, то второй становится `long`;

ЕСЛИ один операнд имеет значение `ulong`, то второй становится ulong. Исключение: Второй не должен быть `sbyte`, `short`, `int`, `long`;

ЕСЛИ один операнд имеет значение `uint`, а второй `sbyte`, `short`, `int`, то они оба примут значение `long`;

ЕСЛИ один операнд имеет значение `uint` то и второй становится `uint`.

ЕСЛИ ни одно из вышеупомянутых условий не выполняется, то ОБА операнда становятся `int`.
Если в выражении участвуют два операнда типа ` byte`, то выражение выведет ответ типа `int`.
Пример:
```csharp
...
```
Все эти приведения происходят лишь внутри выражения, то есть переменная вне выражения остаётся неизменной.
Приведение типов происходит и с унарными операциями:
```csharp
using System;
class PromDemo {
	static void main()
	...
	z = 10;
	//z = -z;//Выдаст ошибку!
	z = (byte) (-z);// Скушает и не поморщится.
	}
```
